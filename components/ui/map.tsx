...
interface MapCircleProps {
  ...
  radius: number;
  /** Fill color of the circle (default: "#3b82f6") */
  color?: string;
  /** Opacity of the circle fill (default: 0.2) */
  opacity?: number;
  /** Stroke color of the circle (default: "#3b82f6") */
  strokeColor?: string;
  /** Stroke width of the circle in pixels (default: 2) */
  strokeWidth?: number;
  /** Stroke opacity of the circle (default: 0.8) */
...
function MapCircle({
  ...
  const getCircleData = useCallback(
    (center: [number, number], radiusInMeters: number) => {
      const points = 64;
      const coords = {
        lng: center[0],
        lat: center[1],
      ...
      const ret = [];
      const distanceX =
        km / (111.32 * Math.cos((coords.lat * Math.PI) / 180));
  ...
  useEffect(() => {
    ...
    map.addSource(sourceId, {
      type: "geojson",
      data: getCircleData(center, radius),
    });
  ...
  }, [isLoaded, map, sourceId, fillLayerId, strokeLayerId]);

  useEffect(() => {
    if (!isLoaded || !map) return;
    const source = map.getSource(sourceId) as MapLibreGL.GeoJSONSource;
    if (source) {
      source.setData(getCircleData(center, radius));
    }
  }, [isLoaded, map, center, radius, sourceId, getCircleData]);

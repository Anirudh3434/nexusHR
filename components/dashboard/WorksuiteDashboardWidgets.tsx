        ...
        {tab === 'leave' ? (
          onLeave.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-8 text-center text-xs text-gray-400">
              <Palmtree size={24} className="mb-2 text-gray-300 dark:text-gray-600" />
          ...
          (joinings.length > 0 || anniversaries.length > 0) ? (
            ...
            <div className="flex flex-col items-center justify-center py-8 text-center text-xs text-gray-400">
              <Award size={24} className="mb-2 text-gray-300 dark:text-gray-600" />
...
export const SprintTasksBoard = ({ tasks = [] }: { tasks?: TaskItem[] }) => {
  ...
  const filteredTasks = tasks.filter(t => {
    ...
    if (filter === 'todo') return t.status === 'to_do';
    if (filter === 'doing') return t.status === 'doing' || t.status === 'in_review';
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'doing':
      case 'in_review':
        return <span className="px-2.5 py-0.5 rounded-lg bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-300 text-[10px] font-bold border border-indigo-200/50">In Review</span>;
      default:
